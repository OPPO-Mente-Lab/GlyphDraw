...
class ImageEmbeddingDataset(wds.DataPipeline, wds.compat.FluidInterface):
    ...
    def preproc(self, sample):
        ...
        # sample["jpg"] = cv2.resize(sample["jpg"], (512, 512), interpolation=cv2.INTER_CUBIC)
        # sample["jpg"] = torch.from_numpy(sample["jpg"]) / 255
        ...
        # 通过裁剪去水印！裁掉1/10的图片。
        # instance_image = instance_image.crop((0, 0, instance_image.size[0], instance_image.size[1] - instance_image.size[1] // 10))
        example["instance_images"] = self.image_transforms(instance_image)
        # print(sample["txt"])
        # print(self.tokenizer.tokenize(sample["txt"]))
        ...
        if "transcript" in sample.keys():
            ...
            example["instance_prompt_ids"] = sample["json"]
            # # else:
            # example["instance_prompt_ids"] = sample["txt"].replace("!","")[:100]
            
            example["font"] = sample["text"].split("\n")[0]
